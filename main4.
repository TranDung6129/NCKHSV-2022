...
model.x = pyo.Var(range(1, 11), range(1, 7), g_set, A_set, B_set, initialize=(0), within=Binary)
x_tignm = model.x

model.y = pyo.Var(A_set, B_set, H_set, range(1, 11), initialize=(0), within=Binary)
y_nmht = model.y

model.z = pyo.Var(B_set, initialize=(0), within=Binary)
z_m = model.z
    ...
    for i in range(1, 7):
        for m in B_set:
            for n in range(len(A_set)):
                x_tignm_components.append(x_tignm[t, i, g_set[n], A_set[n], m])
            x_tignm_sum = sum(x_tignm_components)
        model.constraint_1.add(expr=x_tignm_sum <= 1)
...
for m in B_set:
    for t in range(1, 11):
        for i in range(1, 7):
            x_tignm_components = []
            for g in g_set:
                x_tignm_components.append(x_tignm[t, i, g, A_set[g_set.index(g)], m])
                x_tignm_sum = sum(x_tignm_components)
                model.constraint_2.add(expr=x_tignm_sum <= 1)
    ...
    for m in B_set:
        for t in range(1, 11):
            for i in range(1, 7):
                x_tignm_components.append(x_tignm[t, i, g, A_set[g_set.index(g)], m])
                x_tignm_sum = sum(x_tignm_components)
                model.constraint_3.add(expr=x_tignm_sum == H_set[g_set.index(g)])
# Ràng buộc 4: Các tiết học của một mã lớp phải học trong cùng một phòng 
model.constraint_4 = pyo.ConstraintList()
for n in A_set:
    for m in B_set:
        x_tignm_components_1 = []
        x_tignm_components_2 = []
        for t in range(1, 11):
            for i in range(1, 7):
                x_tignm_components_1.append(x_tignm[t, i, g_set[A_set.index(n)], n, m])
                x_tignm_1_sum = sum(x_tignm_components_1)
                x_tignm_components_2.append(x_tignm[t, i, g_set[A_set.index(n)], n, m])
                x_tignm_2_sum = sum(x_tignm_components_2)
                model.constraint_4.add(expr=H_set[A_set.index(n)] * x_tignm_2_sum <= x_tignm_1_sum)
...
for n in A_set:
    ...
    for m in B_set:
        for t in range(1, 11):
            for i in range(1, 7):
                x_tignm_components.append(x_tignm[t, i, g_set[A_set.index(n)], n, m])
            x_tignm_sum = sum(x_tignm_components)
            model.constraint_6.add(expr=x_tignm_sum == H_set[A_set.index(n)] * y_nmht[n, m, H_set[A_set.index(n)], t])
    ...
    for t in range(1, 11):
        for i in range(1, 7):
            for n in A_set:
                x_tignm_components.append(x_tignm[t, i, g_set[A_set.index(n)], n, m])
                x_tignm_sum = sum(x_tignm_components)
                model.constraint_7.add(expr=z_m[m] <= x_tignm_sum)
                model.constraint_7.add(expr=x_tignm_sum <= 60 * z_m[m])
